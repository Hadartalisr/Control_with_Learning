...
\textcolor{purple}{
\subproblem{a} Write the state-space representation of the inverted pendulum system in the form:
\begin{equation*}
    \dot{\mathbf{y_t}} = A_t \mathbf{y_t} + B_t u_t
\end{equation*}
where \( \mathbf{y_t} = [x_t, \dot{x_t}, \theta_t, \dot{\theta_t}]^T \) is the state vector, and \( A \) and \( B \) are the state and input matrices, respectively.
}
...
\begin{equation*}
    ...
    \end{bmatrix} = \begin{bmatrix}
        0 & 1 & 0 & 0 \\
        0 & -\frac{\delta}{M + m (1 - \cos^2(\theta_t))} & \frac{m g L \sin(\theta_t)}{M + m (1 - \cos^2(\theta_t))} & 0 \\
        0 & 0 & 0 & 1 \\
        0 & -\frac{m L \delta \cos(\theta_t)}{M L + m L (1 - \cos^2(\theta_t))} & \frac{(M + m) g \sin(\theta_t)}{M L + m L (1 - \cos^2(\theta_t))} & 0
    \end{bmatrix} \begin{bmatrix}
        x_t \\
        \dot{x_t} \\
        \theta_t \\
    ...
    \end{bmatrix} + \begin{bmatrix}
        0 \\
        \frac{1}{M + m (1 - \cos^2(\theta_t))} \\
        0 \\
        \frac{\cos(\theta)}{M L + m L (1 - \cos^2(\theta_t))}
    \end{bmatrix} u
\end{equation*}
}
...
\textcolor{purple}{
\subproblem{b} Implement the function \texttt{dynamics} in the provided Python file \texttt{inverted\_pendulum.py}.
}
...
\textcolor{purple}{
    \subproblem{b} Are there exist matrices $A$ and $B$ such that $\forall t \in \mathbb{R}_{\geq 0}$,  $\dot{\mathbf{y_t}} = A_t \mathbf{y_t} + B_t u_t = A \mathbf{y_t} + B u_t $? Justify your answer.
}

\medbreak

\textcolor{teal}{
    No, the matrices $A$ and $B$ cannot be constant for all time $t$ since the system dynamics are nonlinear and depend on the state variables $x$ and $\theta$.
}
